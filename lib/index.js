'use strict';

/**
 * Module dependencies.
 */

var integration = require('@segment/analytics.js-integration');
var del = require('obj-case').del;
var each = require('@ndhoule/each');

/**
 * Expose `MoEngage` integration.
 */

var MoEngage = module.exports = integration('MoEngage')
  .option('appId', '')
  .tag('<script src="https://cdn.moengage.com/webpush/moe_webSdk.min.latest.js">');

/**
 * Initialize.
 *
 * @api public
 */

MoEngage.prototype.initialize = function() {
  var self = this;
  // @hankim - modified this shim to use `t`, `q`, `f`, and `k` as closed variables to abide by strict mode
  /* eslint-disable */
  (function(i, s, o, g, r, a, m, n) {
    i['moengage_object'] = r;
    var t = {};
    var q = function(f) {
      return function() {
        (i['moengage_q'] = i['moengage_q'] || []).push({
          f: f,
          a: arguments
        });
      };
    };
    var f = ['track_event', 'add_user_attribute', 'add_first_name', 'add_last_name', 'add_email', 'add_mobile',
    'add_user_name', 'add_gender', 'add_birthday', 'destroy_session', 'add_unique_user_id', 'moe_events', 'call_web_push', 'track'
    ];
    for (var k in f) {
      t[f[k]] = q(f[k]);
    }
    i['moe'] = i['moe'] || function() {
      n = arguments[0];
      return t;
    };
  })(window, document, 'script', null, 'Moengage');
  /* eslint-enable */

  this.load(function() {
    self._client = window.moe({
      app_id: self.options.appId,
      debug_logs: 1 // TODO: set as 0 before deploy!
    });
    self.ready();
  });
};

/**
 * Loaded?
 *
 * @api public
 * @return {boolean}
 */

MoEngage.prototype.loaded = function() {
  return !!(window.moe.push && Array.prototype.push !== window.moe.push);
};

/**
 * Identify
 *
 * TODO: // document how to logout
 * @api public
 */

MoEngage.prototype.identify = function(identify) {
  if (identify.userId()) this._client.add_unique_user_id(identify.userId());

  // send common traits
  this._client.add_first_name(identify.firstName());
  this._client.add_last_name(identify.lastName());
  this._client.add_email(identify.email());
  this._client.add_mobile(identify.phone());
  this._client.add_user_name(identify.username());
  this._client.add_gender(identify.gender());
  this._client.add_birthday(identify.birthday());

  // send custom traits
  var customTraits = identify.traits();
  del(customTraits, 'firstName');
  del(customTraits, 'lastName');
  del(customTraits, 'name');
  del(customTraits, 'email');
  del(customTraits, 'phone');
  del(customTraits, 'username');
  del(customTraits, 'gender');
  del(customTraits, 'birthday');

  each(function(value, key) {
    this._client.add_user_attribute(key, value);
  }, customTraits);
};

/**
 * Track
 *
 * @api public
 */

MoEngage.prototype.track = function(track) {
  this._client.track_event(track.event(), track.properties());
};
