...
MoEngage.prototype.identify = function(identify) {
  ...
  // send common traits
  this._client.add_first_name(identify.firstName());
  this._client.add_last_name(identify.lastName());
  this._client.add_email(identify.email());
  this._client.add_mobile(identify.phone());
  this._client.add_user_name(identify.name() || identify.username()); // MoEngage specifically asked to map `name` since that's how their current users use this for
  this._client.add_gender(identify.gender());
  this._client.add_birthday(identify.birthday());

  // send custom traits
  var customTraits = identify.traits();
  del(customTraits, 'firstName');
  del(customTraits, 'lastName');
  del(customTraits, 'name');
  del(customTraits, 'email');
  del(customTraits, 'phone');
  del(customTraits, 'gender');
  del(customTraits, 'birthday');
  del(customTraits, 'id');
  if (!identify.name()) del(customTraits, 'username'); // if `traits.name` exists, it was sent on line 101 and thus customer probably wants `traits.username` somewhere
